...
Nếu tháng 2 thì yêu cau nhap thêm năm:
...
// Nhập tháng
let month = parseInt(prompt("Nhập tháng từ 1-12: "));

// Kiểm tra hợp lệ
console.log(isNaN(month));
console.log(!isNaN(month));

// Kiểm tra xem nhập đúng từ 1 - 12 hay chưa
if (!isNaN(month) && month >= 1 && month <= 12) {
  // Nếu là tháng 2 thì yêu cầu nhập thêm năm
  if (month === 2) {
    let nam = parseInt(prompt("Nhập vào năm: "));
    // Check năm xem có phải năm nhuận không
    let checkNamNhuan = (nam % 4 === 0 && nam % 100 !== 0) || nam % 400 === 0;
    console.log("Check năm nhuận là: " + checkNamNhuan);
    if (checkNamNhuan) {
      console.log("Tháng " + month + " năm " + nam + " có 29 ngày");
    } else {
      console.log("Tháng " + month + " năm " + nam + " có 28 ngày");
    }
